...
pub fn renderPubMember(
    ...
    switch (tree.nodes.items(.tag)[decl]) {
        ...
        .container_field_init => {
            try renderDocComments(ais, tree, tree.firstToken(decl));
            try renderContainerField(gpa, ais, tree, tree.containerFieldInit(decl), is_tuple, space);
        },
        .container_field_align => {
            try renderDocComments(ais, tree, tree.firstToken(decl));
            try renderContainerField(gpa, ais, tree, tree.containerFieldAlign(decl), is_tuple, space);
        },
        .container_field => {
            try renderDocComments(ais, tree, tree.firstToken(decl));
            try renderContainerField(gpa, ais, tree, tree.containerField(decl), is_tuple, space);
        },
        else => |tag| {
...
fn renderExpressions(gpa: Allocator, ais: *Ais, tree: Ast, expressions: []const Ast.Node.Index, space: Space) !void {
...
fn renderExpression(gpa: Allocator, ais: *Ais, tree: Ast, node: Ast.Node.Index, space: Space) anyerror!void {
    const token_tags = tree.tokens.items(.tag);
    const main_tokens = tree.nodes.items(.main_token);
...
fn renderIf(gpa: Allocator, ais: *Ais, tree: Ast, if_node: Ast.full.If, space: Space) !void {
...
fn renderWhile(gpa: Allocator, ais: *Ais, tree: Ast, while_node: Ast.full.While, space: Space) !void {
    const node_tags = tree.nodes.items(.tag);
    const token_tags = tree.tokens.items(.tag);
...
fn renderFnProto(gpa: Allocator, ais: *Ais, tree: Ast, fn_proto: Ast.full.FnProto, space: Space) !void {
    const token_tags = tree.tokens.items(.tag);
...
fn renderContainerDecl(
    ...
    if (is_tuple) for (container_decl.ast.members) |member| {
        const tuple_like = switch (node_tags[member]) {
    ...
    if (!src_has_trailing_comma) one_line: {
        ...
        for (container_decl.ast.members) |member| {
            try renderPubMember(gpa, ais, tree, member, is_tuple, .space);
        }
        return renderToken(ais, tree, rbrace, space); // rbrace
    }
    ...
    for (container_decl.ast.members) |member, i| {
        ...
        switch (tree.nodes.items(.tag)[member]) {
            ...
            .container_field_align,
            .container_field,
            => try renderPubMember(gpa, ais, tree, member, is_tuple, .comma),

            else => try renderPubMember(gpa, ais, tree, member, is_tuple, .newline),
        }
    }
    ais.popIndent();
...
fn renderExpressionIndented(gpa: Allocator, ais: *Ais, tree: Ast, node: Ast.Node.Index, space: Space) !void {
    const token_starts = tree.tokens.items(.start);
    const token_tags = tree.tokens.items(.tag);
...
fn renderExpressionComma(gpa: Allocator, ais: *Ais, tree: Ast, node: Ast.Node.Index, space: Space) !void {
    const token_tags = tree.tokens.items(.tag);
...
/// If a comma is present, and `space` is `Space.comma`, render only a single comma.
fn renderTokenComma(ais: *Ais, tree: Ast, token: Ast.TokenIndex, space: Space) !void {
    const token_tags = tree.tokens.items(.tag);
...
/// If a comma is present, and `space` is `Space.comma`, render only a single comma.
fn renderIdentifierComma(ais: *Ais, tree: Ast, token: Ast.TokenIndex, space: Space, quote: QuoteBehavior) !void {
    const token_tags = tree.tokens.items(.tag);
    const maybe_comma = token + 1;
    if (token_tags[maybe_comma] == .comma and space != .comma) {
...
fn renderToken(ais: *Ais, tree: Ast, token_index: Ast.TokenIndex, space: Space) anyerror!void {
    const lexeme = tokenSliceForRender(tree, token_index);
    try ais.writer().writeAll(lexeme);
    try renderSpace(ais, tree, token_index, lexeme.len, space);
}

fn renderSpace(ais: *Ais, tree: Ast, token_index: Ast.TokenIndex, lexeme_len: usize, space: Space) anyerror!void {
    const token_tags = tree.tokens.items(.tag);
    const token_starts = tree.tokens.items(.start);
...
fn renderIdentifier(ais: *Ais, tree: Ast, token_index: Ast.TokenIndex, space: Space, quote: QuoteBehavior) !void {
    const token_tags = tree.tokens.items(.tag);
...
fn renderComments(ais: *Ais, tree: Ast, start: usize, end: usize) anyerror!bool {
...
fn renderExtraNewlineToken(ais: *Ais, tree: Ast, token_index: Ast.TokenIndex) anyerror!void {
    const token_starts = tree.tokens.items(.start);
    const token_start = token_starts[token_index];
...
fn renderDocComments(ais: *Ais, tree: Ast, end_token: Ast.TokenIndex) anyerror!void {
...
pub const TypeErasedWriter = struct {
    ...
    const VTable = struct {
        writeFn: *const fn (usize, []const u8) WriteError!usize,
    };
